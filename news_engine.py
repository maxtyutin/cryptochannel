...
Напиши краткую новостную заметку на русском языке для публикации в Telegram-канале на основе следующего текста:
...
2. Тело поста: 3-4 лаконичных предложения, объясняющих суть события без лишней "воды".
3. В конце добавь раздел "Что это значит для рынка? 🤔" с выводом в 1-2 предложения (будет рост, падение или нейтрально для цены активов).
        ...
        if bot_token and chat_id:
            ...
            if selected_item.get('image_url'):
                ...
                if success:
                    ...
                    print("Не удалось отправить с изображением. Пробуем отправить только текст...")
