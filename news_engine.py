...
GEMINI_MODELS = [
    "gemini-2.5-flash",        # Основная стабильная модель
    "gemini-2.5-flash-lite",   # Высокая скорость
    "gemini-flash-latest",     # Динамический альтернативный эндпоинт
    "gemini-flash-lite-latest",# Резервный динамический эндпоинт
    "gemini-2.0-flash"         # Классическая модель
]
...
"""
    ...
    try:
        with open(rss_path, 'w', encoding='utf-8') as f:
            f.write(rss_xml)
        print(f"[dzen] Успешно обновлена RSS-лента Яндекс Дзен ({len(articles[:50])} статей) -> {rss_path}")
    except Exception as e:
        print(f"[dzen] Ошибка генерации dzen.xml: {e}")

def save_article_to_json(news_item, post_text, russian_title=None, category="news", telegram_caption=None):
    """Сохраняет опубликованную новость в файл articles.json для веб-сайта и обновляет dzen.xml"""
    json_path = os.path.join(BASE_DIR, "articles.json")
    articles = []
    if os.path.exists(json_path):
        try:
...
def main():
    ...
    for item in news_list:
        ...
        if pat:
            ...
            git_sync_and_push(pat, "Auto-update website database [skip ci]", ["articles.json", "dzen.xml", "images/"])
